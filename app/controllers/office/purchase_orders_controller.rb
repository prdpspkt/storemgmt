class Office::PurchaseOrdersController < OfficeController
  ...
  def print
    ...
    @amount = current(Office::PurchaseOrderItem).where(purchase_order_id: @purchase_order.id).sum(:amount)
    @vat = current(Office::PurchaseOrderItem).where(purchase_order_id: @purchase_order.id).sum(:vat)
    @amount_without_vat = current(Office::PurchaseOrderItem).where(purchase_order_id: @purchase_order.id).sum(:amount_without_vat)
