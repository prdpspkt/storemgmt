class Project::ItemEvaluationItemsController < ProjectController
  before_action :set_evaluation_item, only: [:edit, :update]

  def edit
    @item_evaluation_item = Project::ItemEvaluationItem.find(params[:id])
    render layout: 'modal'
  end
  ...
  def evaluation_item_params
    params.require(:project_item_evaluation_item).permit(:unmatched, :dquantity, :iquantity, :diquantity, :notworking, :to_be_repaired, :to_be_auctioned, :to_be_disposed, :to_be_conserved, :remarks )
  end
end
