class ProjectPurchaseOrdersController < ApplicationController
  ...
  def index
    @purchase_orders = PurchaseOrder.where(user_id: current_user.id, project_id: current_project.id, fiscal_year_id: current_fiscal_year.id)
  end
  ...
  def new
    ...
    @vendors = Vendor.where(user_id: current_user.id).where(project_id: current_project.id).where(fiscal_year_id: current_fiscal_year.id)
  end
  ...
  def create
    ...
    @purchase_order.project_cheif_name = current_project_chief.name_ne
    ...
    @purchase_order.project_id = current_project.id
    @purchase_order.fiscal_year_id = current_fiscal_year.id
    @purchase_order.fy = current_fiscal_year.fy
    @purchase_order.project_name = current_project.project
    @purchase_order.project_address = current_project.address
  ...
  def can_unmark obj
    ...
    def purchase_order_params
      params.require(:purchase_order).permit(:vendor_name, :vendor_address, :vendor_registration, :vendor_phone, :vendor_pan, :order_no, :order_date, :order_decision_no, :order_decision_date, :date_to_receive_goods, :project_name, :project_address, :store_chief_name, :store_chief_signed_date, :section_chief_name, :section_chief_signed_date, :project_cheif_signed_date, :project_cheif_name, :user_id, :fy, :fiscal_year_id, :project_id)
    end
