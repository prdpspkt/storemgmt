class Project::PurchaseOrdersController < ProjectController
  ...
  def print
    ...
    @amount = current(Project::PurchaseOrderItem).where(purchase_order_id: @purchase_order.id).sum(:amount)
    @vat = current(Project::PurchaseOrderItem).where(purchase_order_id: @purchase_order.id).sum(:vat)
    @amount_without_vat = current(Project::PurchaseOrderItem).where(purchase_order_id: @purchase_order.id).sum(:amount_without_vat)
