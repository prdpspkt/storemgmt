class OfficeEvaluationGenerator
  ...
  def perform(data)
    ActionCable.server.broadcast "progress_channel", response: {progress_message: "We are generating evaluation report in background please wait..."}
    ...
    ActionCable.server.broadcast "progress_channel", response: {closed: true, reload: true, message: ""}
  end
