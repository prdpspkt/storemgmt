class FiscalYearClosingWorker
  ...
  def perform(data)
    ...
    items.each do |item|
      transactions = get_office_item_transactions(item.item_id)
      if transactions.count > 0
        create_office_item_transaction transactions
      end
    end
  ...
  def create_project_item project_item
    new_item = Project::ProjectItem.where(office_id: @office.id)
                   ...
                   .where(item_id: project_item.item_id).first
    if new_item.blank?
  ...
  def create_project_item_transaction transactions
    ...
    transaction.save!
    create_office_item_expense_transaction transaction
  end

  def create_project_item_expense_transaction transaction
    transaction1 = Office::ItemTransaction.new(transaction.attributes.select { |key, _| Office::ItemTransaction.column_names.include? key })
    transaction1.id = nil
  ...
  def new_office_item_register_page_no
    item_register_page_no = 1
    items = Office::Item.where(office_id: @office.id).where(fiscal_year_id: @fiscal_year.id)
    if items.count.positive?
      item_register_page_no = items.last.item_register_page_no + 1
    end
    item_register_page_no
  ...
  def create_office_item_transaction transactions
    ...
    transaction.id = nil
    transaction.item_id = create_office_item item
    transaction.rate = transactions.average(:rate)
    transaction.quantity = transactions.sum(:sku)
    transaction.amount = transaction.rate * transaction.quantity
    transaction.transaction_date = @fiscal_year.start_date
  ...
  def create_office_item_expense_transaction transaction
    transaction1 = Office::ItemTransaction.new(transaction.attributes.select { |key, _| Office::ItemTransaction.column_names.include? key })
    transaction1.id = nil
    transaction1.transaction_date = @old_fiscal_year.closing_date
    transaction1.transaction_type = -1
