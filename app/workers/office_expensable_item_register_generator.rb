class OfficeExpensableItemRegisterGenerator
  ...
  def perform office_id, fiscal_year_id, store_body_id
    ...
    items = Office::Item.where(office_id: office_id).where(fiscal_year_id: fiscal_year_id).where(item_classification_no: 407)
