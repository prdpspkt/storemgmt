class ProjectPurchaseEntry < ApplicationRecord
  has_many :project_purchase_entry_items, dependent: :delete_all
  belongs_to :user
  belongs_to :project
  ...
  def create_expensable_item_transaction opei
    oeirt = Oeirt.find_or_create_by({project_purchase_entry_item_id: opei.id})
    oeirt.update({
                     project_item_id: opei.project_item_id,
                     item_id: opei.item_id,
                     project_id: opei.project_id,
                     fiscal_year_id: opei.fiscal_year_id,
                     user_id: opei.user_id,
                     transaction_date: opei.project_purchase_entry.entry_date,
                     transaction_type: 1,
  ...
  def create_non_expensable_item_transaction opei
    obj = Oneirt.find_or_create_by({project_purchase_entry_item_id: opei.id})
    obj.update({
                   project_item_id: opei.project_item_id,
                   item_id: opei.item_id,
                   ...
                   size: opei.size,
                   entry_no: opei.project_purchase_entry.entry_no,
